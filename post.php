		...
		<div id="container">
			...
			<div id="post-area">
				<div id="form-container">
					<p>
						<h3>Welcome to solusi-fi</h3>
					</p>
					<form method="post" style="display:none" onSubmit="return false;">
						 	...
						 	<div class='post'>
								...
								<div class='posted-wraper'>
									...
									<div class="post-stats">
										...
										<time class="timeago pull-right" datetime="<?= $post->psostDate ?>" title="July 17, 2016"></time>
									</div>
									...
									<div class="comments-box" id="view_comments<?= $post->postId ?>" style='display:block;'>
										<div class="form_elements">
										...
										<div id="ajaxresponse<?= $post->postId?>">
						 			 		<?php foreach( $comments as $comment): ?>
						 			 			...
						 			 			<div class='comment'>
													...
													<div class="post-stats">
														<?php if($myL->checkLike($u,"C",$comment->commentId)== true):?>
															<div class='commentinfo-strip pointer'><span onClick='likeComment(<?=$comment->commentId?>,<?=$u?>,<?= $post->userId ?>)' id='like-span-comment<?= $comment->commentId ?>'>unlike</span>
														<?php else:?>		
															<div class='commentinfo-strip pointer'><span onClick='likeComment(<?=$comment->commentId?>,<?=$u?>,<?=$post->userId?>)' id='like-span-comment<?= $comment->commentId?>'>like</span>
														<?php endif;?>	
															</div>
														<a onClick='getLikersComment(<?= $comment->commentId ?>)' rel='<?= $likes ?>' id='comment-num<?= $comment->commentId ?>'><i class='fa fa-thumbs-up fa-fw'></i><?= $likes ?></a>	
														<time class="timeago pull-right" datetime="<?= $post->postDate ?>" title="July 17, 2008"></time>
													</div>
												</div>
											 <?php endforeach; ?>
										</div>
